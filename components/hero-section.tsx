...
export default function HeroSection() {
  ...
  useEffect(() => {
    ...
    function fadeIn(fromOpacity?: number) {
      cancelAnim();
      if (!video) return;
      const currentOpStr = video.style.opacity;
      const startOpacity = fromOpacity ?? (currentOpStr ? parseFloat(currentOpStr) : 0);
    ...
    function fadeOut(fromOpacity?: number) {
      ...
      if (!video) return;
      const currentOpStr = video.style.opacity;
      const startOpacity = fromOpacity ?? (currentOpStr ? parseFloat(currentOpStr) : 1);
      const startTime = performance.now();
      const duration = 500;
